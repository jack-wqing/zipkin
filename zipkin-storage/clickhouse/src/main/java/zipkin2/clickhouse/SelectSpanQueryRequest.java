...
public class SelectSpanQueryRequest implements Function<Client, List<Span>> {
  ...
  public List<Span> apply(Client client) {
    ...
    querySql += Constants.BLANK_CHAR + "AND time >= " + Constants.SINGLE_QUOTA + DateFormatUtils.format(new Date(endTs - lookback), Constants.DATE_FORMAT) + Constants.SINGLE_QUOTA +
      Constants.BLANK_CHAR + "AND time <= " + Constants.SINGLE_QUOTA + DateFormatUtils.format(new Date(endTs), Constants.DATE_FORMAT) + Constants.SINGLE_QUOTA;
    String serviceName = queryRequest.serviceName();
    if (StringUtils.isNotBlank(serviceName)) {
      querySql +=  Constants.BLANK_CHAR + "AND " + Constants.SERVICE_NAME + " = " + Constants.SINGLE_QUOTA + serviceName + Constants.SINGLE_QUOTA;
    }
  ...
  private String condition() {
    ...
    if (StringUtils.isNotBlank(remoteServiceName)) {
      sqlCondition.append(Constants.BLANK_CHAR + "AND remoteEndpoint['serviceName'] = " + Constants.SINGLE_QUOTA + remoteServiceName +  Constants.SINGLE_QUOTA);
    }
    String spanName = queryRequest.spanName();
    if (StringUtils.isNotBlank(spanName)) {
      sqlCondition.append(Constants.BLANK_CHAR + "AND name = " +  Constants.SINGLE_QUOTA + spanName + Constants.SINGLE_QUOTA);
    }
    ...
    if (minDuration != null && minDuration.longValue() != 0 && maxDuration != null && maxDuration.longValue() != 0) {
      sqlCondition.append(" AND " + Constants.DURATION + " >= " + minDuration);
      sqlCondition.append(" AND " + Constants.DURATION + " <= " + maxDuration);
  ...
  public String annotationQueryString() {
    ...
    for (Iterator<Map.Entry<String, String>> i = queryRequest.annotationQuery().entrySet().iterator(); i.hasNext(); ) {
      ...
      result.append(Constants.TAGS + "['" + next.getKey() + "']");
      result.append('=').append(Constants.SINGLE_QUOTA + next.getValue() + Constants.SINGLE_QUOTA);
